...
/// Get default model from config.json or fallback to claude-opus-4-6
fn get_default_model() -> String {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| "/".to_string());
    let config_path = std::path::PathBuf::from(home).join(".claw").join("config.json");

    std::fs::read_to_string(&config_path)
        .ok()
        .and_then(|content| serde_json::from_str::<serde_json::Value>(&content).ok())
        .and_then(|config| config.get("model")?.as_str().map(|s| s.to_string()))
...
impl DefaultRuntimeClient {
    fn new(
        ...
        // Try to get auth source, but don't fail if not available
        // (OpenAI-compatible providers can use config.json instead)
        let auth_source = resolve_cli_auth_source().ok();
        let client = ProviderClient::from_model_with_default_auth(&model, auth_source)?;
        Ok(Self {
                ...
                match event {
                    ApiStreamEvent::MessageStart(start) => {
                        for block in start.message.content {
                            if matches!(block, OutputContentBlock::Thinking { .. }) {
                    ...
                    ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                        ContentBlockDelta::TextDelta { text } => {
                            if !text.is_empty() {
                                // Add newline after thinking ends
                                if thinking_ended {
                                    write!(out, "\n\n")
                                        .and_then(|()| out.flush())
                                        .map_err(|error| RuntimeError::new(error.to_string()))?;
                                ...
                                // Output text directly like thinking - natural flow
                                write!(out, "{text}")
                                    .and_then(|()| out.flush())
                                    .map_err(|error| RuntimeError::new(error.to_string()))?;
                        ...
                        ContentBlockDelta::ThinkingDelta { thinking } => {
                            // Render thinking in gray/dim color
                    ...
                    ApiStreamEvent::ContentBlockStop(_) => {
                        // Mark that thinking ended (will add newline before text)
