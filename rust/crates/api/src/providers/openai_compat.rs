...
/// Config.json structure for ~/.claw/config.json
...
/// Read config from ~/.claw/config.json
fn read_claw_config() -> Option<ClawConfig> {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| "/".to_string());
    let config_path = PathBuf::from(home).join(".claw").join("config.json");

    std::fs::read_to_string(&config_path)
        .ok()
        .and_then(|content| serde_json::from_str::<ClawConfig>(&content).ok())
}

/// Get API key from config.json (for OpenAI-compatible providers)
fn api_key_from_config() -> Option<String> {
    read_claw_config().and_then(|config| config.api_key)
}

/// Get base URL from config.json (for OpenAI-compatible providers)
fn base_url_from_config() -> Option<String> {
    read_claw_config().and_then(|config| config.base_url)
}
...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            // Handle reasoning_content as Thinking block (shown in gray)
            ...
            // Handle content as Text block (normal output)
...
pub fn read_base_url(config: OpenAiCompatConfig) -> String {
    ...
    // Fall back to config.json
    if let Some(url) = base_url_from_config() {
        return url;
    }
